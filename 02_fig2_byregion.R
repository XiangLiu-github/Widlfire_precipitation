...
fire_buffered = qread('data/outputs/fire_buffer_0.25.qs') %>% 
  ...
  select(- ig_date) %>% 
  mutate(regn = str_c(fifelse(centroid_x >= -115, 'right', 'left'), fifelse(centroid_y >= 40, 'top', 'bottom')))
...
get_res = function(amdata, avar, amodel){
  ...
  amdata = amdata %>% 
    mutate(regn = replace_na(regn, 'NA'))
...
p15 = map2(ress,
           ...
           function(ares, alabel){
             ...
             ares = ares %>% 
               ...
               add_row(lc = c("righttop", "lefttop", "leftbottom", "rightbottom"), term = -17, estimate = 0, std.error = 0)
...
ggsave('figures/fig2_byregion.pdf', fp, width = 5, height = 4, scale = 3.85)
